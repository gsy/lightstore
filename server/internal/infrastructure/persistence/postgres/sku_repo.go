...
// PostgresSKURepository implements sku.SKURepository
...
func (r *PostgresSKURepository) Save(ctx context.Context, s *sku.SKU) error {
	var imageURL *string
	if s.ImageURL() != "" {
		url := s.ImageURL()
		imageURL = &url
...
func (r *PostgresSKURepository) FindByID(ctx context.Context, id shared.SKUID) (*sku.SKU, error) {
...
func (r *PostgresSKURepository) FindByCode(ctx context.Context, code string) (*sku.SKU, error) {
	row := r.pool.QueryRow(ctx, `
		SELECT id, code, name, price_cents, currency, weight_grams, weight_tolerance, image_url, active, created_at, updated_at
...
func (r *PostgresSKURepository) FindAllActive(ctx context.Context) ([]*sku.SKU, error) {
...
func (r *PostgresSKURepository) FindAll(ctx context.Context) ([]*sku.SKU, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT id, code, name, price_cents, currency, weight_grams, weight_tolerance, image_url, active, created_at, updated_at
...
func (r *PostgresSKURepository) scanSKU(row pgx.Row) (*sku.SKU, error) {
	var rec skuRow
	err := row.Scan(
...
func (r *PostgresSKURepository) scanSKUs(rows pgx.Rows) ([]*sku.SKU, error) {
	var skus []*sku.SKU
	for rows.Next() {
		var rec skuRow
		err := rows.Scan(
...
func (r *PostgresSKURepository) reconstitute(rec skuRow) *sku.SKU {
	id, _ := shared.SKUIDFrom(rec.ID)
	price, _ := shared.NewMoney(rec.PriceCents, rec.Currency)
	weight, _ := shared.NewWeight(rec.WeightGrams)
