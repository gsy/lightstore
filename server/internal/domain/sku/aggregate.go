...
func NewSKU(code, name string, priceCents int64, currency string, weightGrams float64) (*SKU, error) {
	...
	price, err := shared.NewMoney(priceCents, currency)
	if err != nil {
		return nil, ErrInvalidSKUPrice
	}

	weight, err := shared.NewWeight(weightGrams)
	if err != nil {
	...
	s.events = append(s.events, SKUCreated{
		SKUID: s.id,
		Code:  code,
		Name:  name,
	})
...
// Getters
func (s *SKU) ID() shared.SKUID        { return s.id }
func (s *SKU) Code() string            { return s.code }
func (s *SKU) Name() string            { return s.name }
func (s *SKU) Price() shared.Money     { return s.price }
func (s *SKU) Weight() shared.Weight   { return s.weight }
...
func (s *SKU) Update(name string, priceCents int64, currency string, weightGrams, weightTolerance float64, imageURL string) error {
	...
	s.events = append(s.events, SKUUpdated{
		SKUID: s.id,
		Name:  name,
	})
...
func (s *SKU) Deactivate() {
	...
	s.active = false
	s.updatedAt = time.Now().UTC()
	s.events = append(s.events, SKUDeactivated{SKUID: s.id})
}
...
func (s *SKU) IsWeightMatch(measured shared.Weight) bool {
...
func (s *SKU) PullEvents() []DomainEvent {
	events := s.events
	s.events = nil
	return events
}
